...
LONG_MESSAGE += ("""I know of a song that gets on everybody's nerves, everybody's nerves, everybody's nerves. 
I know of a song that gets on everybody's nerves and this is how it goes.""" * 10)
...
def format_cli_args(message_content, message_title="", webhook_url="", team_id="", channel_id=""):
    return [
        "post_message.py",
...
class TestWebhook:
    ...
    def test_happy_path(self, monkeypatch, message_content, message_title):
        ...
        with monkeypatch.context() as patcher:
            patcher.setattr(sys, 'argv', format_cli_args(
    ...
    def test_webhook_url_missing(self, monkeypatch):
        ...
        with (monkeypatch.context() as patcher):
            patcher.setattr(
                sys, 'argv', format_cli_args(
                    message_content=SHORT_MESSAGE,
            ...
            try:
                ...
                assert e.code == MicrosoftTeamsClient.EXIT_CODE_INVALID_CREDENTIALS, f'Expected {MicrosoftTeamsClient.EXIT_CODE_INVALID_CREDENTIALS} but got {e.code}'
