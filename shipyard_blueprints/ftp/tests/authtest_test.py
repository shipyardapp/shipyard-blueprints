...
from shipyard_ftp.cli.authtest import main
...
def test_invalid_password(invalid_password, monkeypatch):
    monkeypatch.setenv("FTP_PASSWORD", invalid_password)
    with pytest.raises(SystemExit) as exit_code:
        main()
...
def test_invalid_username(invalid_username, monkeypatch):
    monkeypatch.setenv("FTP_USERNAME", invalid_username)
    with pytest.raises(SystemExit) as exit_code:
        main()
    assert exit_code.value.code == 1
...
def test_invalid_host(invalid_host, monkeypatch):
    monkeypatch.setenv("FTP_HOST", invalid_host)
    with pytest.raises(SystemExit) as exit_code:
        main()
    assert exit_code.value.code == 1


@pytest.mark.parametrize("invalid_port", ["bad_port", "300"])
def test_invalid_port(invalid_port, monkeypatch):
    monkeypatch.setenv("FTP_PORT", invalid_port)
    with pytest.raises(SystemExit) as exit_code:
        main()
    assert exit_code.value.code == 1


@pytest.mark.parametrize("missing_env", ["FTP_HOST", "FTP_USERNAME", "FTP_PASSWORD"])
