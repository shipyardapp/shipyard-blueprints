...
def connect_to_slack():
    ...
    """
    slack_connection = WebClient(os.environ.get("SLACK_BOT_TOKEN"), timeout=120)
...
def send_slack_message(slack_connection: WebClient, message, channel_name, blocks):
    ...
    print(f'Your message of "{message}" was successfully sent to {channel_name}')
...
def slack_user_id_lookup(slack_connection, name_to_lookup, user_lookup_method):
    ...
    for attempt in range(tries):
        try:
            ...
            if user_id == "":
                print(
                    'The name of "{}" does not exist in Slack.'.format(name_to_lookup)
                )
...
def update_slack_message(slack_connection, message, channel_id, blocks, timestamp):
    ...
    print(f"Your message was updated")
    return message
