...
def get_args():
    ...
    parser.add_argument('--resource-subtype', dest = 'resource_subtype', required = False, default = 'default_task', choices = {'default_task', 'milestone','section','approval'})
    parser.add_argument('--approval-status', dest = 'approval_status', required = False, default = 'pending', choices = {'pending','approved','rejected','changes_requested'})
...
def main():
    ...
    try:
        if project_id != '':
            ...
            logger.info(f"No project id provided, creating task without project in workspace {workspace_id}")
        result = client.tasks.create_task(params)
        logger.info(f"Task successfully created with gid: {result['gid']} located at {result['permalink_url']}'")
