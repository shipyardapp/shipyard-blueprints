...
def get_run_status(project_id, api_token, run_id):
    ...
    """
    pattern = re.compile(
        "^[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-4[0-9a-fA-F]{3}-[89ABab][0-9a-fA-F]{3}-[0-9a-fA-F]{12}$")  ## copied from Hex's error message
...
def main():
    ...
    status_data_file = shipyard.files.combine_folder_and_file_name(artifact_subfolder_paths['responses'],
                                                                   f'run_{run_id}_status_response.json')
