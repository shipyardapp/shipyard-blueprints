...
class DomoClient(DataVisualization):
    def __init__(self, client_id: str = None, secret_key: str = None,
                 access_token: str = None, domo_instance=None) -> None:
    ...
    def connect_with_client_id_and_secret_key(self):
        try:
            self.domo_client = Domo(self.client_id, self.secret_key, api_host='api.domo.com')
    ...
    def connect_with_access_token(self):
        try:
            ...
            response = requests.get(f'https://{self.domo_instance}/api/content/v1/cards',
    ...
    def connect(self):
        ...
        if check_access_token and check_client_id_secret:
            print('Both Client ID and Secret Key and Access Token and Domo Instance ID were provided.')
            return 1 if self.connect_with_client_id_and_secret_key() or self.connect_with_access_token() == 1 else 0
            ...
            print("Be sure to provide Client ID and Secret Key or Access Token and Domo Instance ID")
            return 1
