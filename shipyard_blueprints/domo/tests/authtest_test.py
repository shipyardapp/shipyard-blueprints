...
class DomoClientConnectTestCase(unittest.TestCase):
    ...
    def test_connect_with_valid_credentials(self):
        ...
        assert (
            DomoClient(client_id=self.client_id, secret_key=self.secret_key).connect()
            == 0
        )
        assert (
            DomoClient(
                client_id=self.client_id, secret_key=self.secret_key, domo_instance=""
            ).connect()
            == 0
        )
        assert (
        ...
        assert (
            DomoClient(
                client_id=self.client_id, secret_key=self.secret_key, access_token=""
            ).connect()
            == 0
        )
        ...
        assert (
            DomoClient(
                access_token=self.access_token, domo_instance=self.domo_instance
            ).connect()
            == 0
        )
        assert (
    ...
    def test_connect_with_invalid_client_id(self):
        assert (
            DomoClient(client_id="invalid", secret_key=self.secret_key).connect() == 1
        )
    ...
    def test_connect_with_credentials_swapped(self):
        assert (
            DomoClient(client_id=self.secret_key, secret_key=self.client_id).connect()
            == 1
        )
    ...
    def test_connect_with_invalid_access_token(self):
        assert (
            DomoClient(
                access_token="invalid", domo_instance=self.domo_instance
            ).connect()
            == 1
        )
        assert (
            DomoClient(access_token="", domo_instance=self.domo_instance).connect() == 1
        )

    def test_connect_with_invalid_domo_instance_id(self):
        assert (
            DomoClient(
                access_token=self.access_token, domo_instance="invalid"
            ).connect()
            == 1
        )
        assert (
            DomoClient(access_token=self.access_token, domo_instance="").connect() == 1
        )
