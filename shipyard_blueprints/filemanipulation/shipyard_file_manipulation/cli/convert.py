...
from shipyard_file_manipulation.core import convert
from shipyard_file_manipulation.errors import ConversionError
...
#
# def convert_file(
#     source_full_path, destination_file_format, destination_full_path, **extra_args
# ):
    ...
    if match_type == "regex_match":
        ...
        if len(matching_file_names) > 0:
            ...
            for index, key_name in enumerate(matching_file_names):
                ...
                print(
                    f"Converting file {index+1} of {len(matching_file_names)} to {target_format}"
                )
                try:
                    convert(
                        src_path=key_name,
                    ...
                    # convert_file(
                    #     source_full_path=key_name,
                    #     destination_file_format=target_format,
                    #     destination_full_path=target_path,
                    #     **extra_args,
                    # )
                except Exception as e:
                    print(f"Could not convert {src_path}")
                    print(e)
        else:
            print("No matches were found.")

    else:
        target_path = determine_destination_full_path(
            destination_folder_name=target_dir,
            destination_file_name=args.destination_file_name,
