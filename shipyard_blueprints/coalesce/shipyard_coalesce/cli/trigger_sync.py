...
def get_args():
    ...
    parser.add_argument('--access-token', dest = 'access_token',
                        required= True)
    parser.add_argument('--environment-id', dest = 'enviornment_id', required = True)
    parser.add_argument('--job-id', dest = 'job_id', required = False, default= None)
    parser.add_argument('--snowflake-username',dest = 'snowflake_username' ,required = True)
    parser.add_argument('--snowflake-password', dest = 'snowflake_password', required = True)
...
def main():
    ...
    environment_id = args.enviornment_id
    job_id = args.job_id
    ...
    response = client.trigger_sync(environment_id = environment_id, job_id = job_id, snowflake_username = snowflake_username,
                            snowflake_password = snowflake_password, snowflake_warehouse = snowflake_warehouse,
                            snowflake_role = snowflake_role, parallelism = parallelism, include_nodes_selector = include_nodes, exclude_nodes_selector = exclude_nodes)
