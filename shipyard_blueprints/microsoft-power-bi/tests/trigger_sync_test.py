...
def run_command():
    def _run_command():
        ...
        arg_list = [
            "--client-id",
            os.getenv("MICROSOFT_POWER_BI_CLIENT_ID", ""),  # Optional
            "--client-secret",
            os.getenv("MICROSOFT_POWER_BI_CLIENT_SECRET", ""),  # Optional
            "--tenant-id",
            os.getenv("MICROSOFT_POWER_BI_TENANT_ID", ""),  # Optional
            "--group-id",
            os.getenv("MICROSOFT_POWER_BI_GROUP_ID", ""),  # Required
            "--object-type",
            os.getenv("MICROSOFT_POWER_BI_OBJECT_TYPE", ""),  # Required
            "--object-id",
            os.getenv("MICROSOFT_POWER_BI_OBJECT_ID", ""),  # Required
            "--wait-for-completion",
            os.getenv("MICROSOFT_POWER_BI_WAIT_FOR_COMPLETION", "false"),
            "--poke-interval",
            "1",
        ]
...
class TestDataflowSync:
    ...
    def test_dataflow_sync_bad_credentials(self, run_command, monkeypatch):
        ...
        assert (
            result.returncode != 0
        ), "Process unexpectedly succeeded with missing credentials."
        assert (
            result.returncode == MicrosoftPowerBiClient.EXIT_CODE_INVALID_CREDENTIALS
        ), "Unexpected exit code."

        monkeypatch.setenv("MICROSOFT_POWER_BI_CLIENT_ID", "bad_id")
        assert (
            result.returncode != 0
        ), "Process unexpectedly succeeded with bad credentials."
        assert (
            result.returncode == MicrosoftPowerBiClient.EXIT_CODE_INVALID_CREDENTIALS
        ), "Unexpected exit code."
    ...
    def test_dataflow_sync_bad_id(self, run_command, monkeypatch):
        ...
        assert (
            result.returncode != 0
        ), "Process unexpectedly succeeded with bad object_id."
        ...
        assert (
            result.returncode != 0
        ), "Process unexpectedly succeeded with missing object_id."
