...
class ClickupClient(ProjectManagement):
    ...
    def _requests(self, endpoint: str, method: str = "GET", payload: dict = None):
        ...
        if error_message == "Token invalid":
            raise ExitCodeError(
                "Invalid Credentials: Please check your key and try again",
                self.EXIT_CODE_INVALID_CREDENTIALS,
            )
        elif error_message == "Status not found":
            raise ExitCodeError(
                "Invalid Status: Check for typos and try again",
                self.EXIT_CODE_INVALID_STATUS,
            )
    ...
    def create_ticket(
        ...
        **kwargs,
        ...
        if due_date:
            due_date = (
                datetime.datetime.strptime(due_date, "%Y-%m-%d").timestamp() * 1000
            )
        ...
        response = self._requests(
            endpoint=f"list/{list_id}/task", payload=payload, method="POST"
        )
    ...
    def update_ticket(
        ...
        **kwargs,
        ...
        payload = {key: value for key, value in data.items() if value is not None}
        response = self._requests(
            endpoint=f"task/{task_id}", payload=payload, method="PUT"
        )
