...
def get_args():
    ...
    parser.add_argument(
        "--access-token", required=True, help="Access token for ClickUp API"
    )
    ...
    parser.add_argument(
        "--notify_all", help="If you wish to notify all assignees, set this to True"
    )
...
def main():
    ...
    try:
        clickup.add_comment(
            task_id=args.task_id, comment=args.comment, notify_all=notify_all
        )
