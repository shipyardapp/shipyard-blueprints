...
def conn_helper(client: DatabricksClient) -> int:
    try:
        response = requests.get(f"https://{client.instance_url}/api/2.0/clusters/list",headers = client.headers)
...
def test_good_connection():
    client = DatabricksClient(access_token= os.getenv('DATABRICKS_ACCESS_TOKEN'),
...
def test_bad_token():
    client = DatabricksClient(access_token= 'bad_token',
                              instance_url= os.getenv('DATABRICKS_INSTANCE_URL'))
