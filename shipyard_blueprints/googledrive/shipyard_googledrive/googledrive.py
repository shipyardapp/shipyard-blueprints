...
class GoogleDriveClient(CloudStorage):
    ...
    def _get_folder_id(self, folder_name: Optional[str]) -> Union[str, None]:
        """Given the name of the folder, this will fetch the ID based off of a match
        Args:
        ...
        try:
            if self.drive_id:
                ...
                results = (
                    self.service.files()
                    .list(q=str(query), fields="files(id,name)")
                    .execute()
                )
            folder_id = None
            _folder = results.get("files", [])
            if _folder != []:
                folder_id = _folder[0].get("id")

        except Exception:
            # NOTE: If any exception occurs, the folder ID will be set to None
            self.logger.warning(
                "An error occurred when attempting to fetch the folder ID...setting to None"
            )
            return None
    ...
    def _get_drive_id(self, drive_name: Optional[str]) -> Union[str, None]:
        """Helper function to grab the ID of the shared drive based off of the name. returns None if there is no match.

        NOTE: The comparison between names is case sensitive

        Args:
            drive_name: The name of the drive to search for

        Returns: The ID or None if no match is found
