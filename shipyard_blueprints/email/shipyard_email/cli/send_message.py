...
def main():
    ...
    client = EmailClient(smtp_host= smtp_host, smtp_port= smtp_port,
                         username = username, password = password, send_method= send_method)
    ...
    if EmailClient.should_message_be_sent(
        ...
        if conditional_send == 'file_dne':
            ...
            client.logger.error("File(s) were found, but message was conditional based on file not existing. Message not sent")
