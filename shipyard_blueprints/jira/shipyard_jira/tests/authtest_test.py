...
class JiraClientConnectTestCase(unittest.TestCase):
    ...
    def test_connect_with_valid_credentials(self):
        assert JiraClient(self.access_token, self.domain, self.email_address).connect() == 0

    def test_connect_with_invalid_domain(self):
        assert JiraClient(self.access_token, 'invalid_domain', self.email_address).connect() == 1
        assert JiraClient(self.access_token, '', self.email_address).connect() == 1


    def test_connect_with_invalid_email(self):
        assert JiraClient(self.access_token, self.domain, 'invalid_email').connect() == 1
    ...
    def test_connect_with_invalid_access_token(self):
        assert JiraClient('invalid_token', self.domain, self.email_address).connect() == 1
    ...
    def test_connect_with_valid_access_token_and_invalid_email_and_domain(self):
        assert JiraClient(self.access_token, 'invalid_domain', 'invalid_email').connect() == 1
