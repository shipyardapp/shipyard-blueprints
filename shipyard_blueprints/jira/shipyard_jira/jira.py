...
class JiraClient(ProjectManagement):
    def __init__(self, access_token: str, domain: str, email_address: str):
        logging.basicConfig(
            level=logging.INFO,
    ...
    def _request(self, endpoint: str, method: str = "GET", data: dict = None) -> dict:
        ...
        if response.status_code == 204:
            logging.info(
                "Successfully completed request. Response has no content as expected"
            )
            return {}
    ...
    def _error_handler(self, status_code: str, error_details: dict) -> None:
        """
        ...
        **kwargs,
        ...
        if assignee not in ("-1", None):
            ...
            try:
                ...
                logging.error(
                    "Assignee not found. Please verify that the email address is correct."
                )
        ...
        try:
            response = self._request("issue", method="POST", data=payload)
        except Exception as error:
    ...
    def create_subtask(
        ...
        **kwargs,
        ...
        task_details = {"parent": {"key": parent_ticket_id}, **kwargs}
        response = self.create_ticket(
            project_key, summary, issue_type, description, assignee, **task_details
        )
        return response

    def update_ticket(self, ticket_key: str, **kwargs) -> dict:
        """
        ...
        logging.info(data)
        try:
            response = self._request(f"issue/{ticket_key}", method="PUT", data=data)
        except Exception as error:
    ...
    def find_user_by_email_address(self, email: str) -> dict:
        """
    ...
    def retrieve_account_id_by_email(self, email: str) -> str:
        """
        :param email: The email address of the user to be retrieved
    ...
    def add_comment(self, ticket_key: str, comment: str) -> dict:
        ...
        try:
            response = self._request(
                f"issue/{ticket_key}/comment", method="POST", data={"body": comment}
            )
        except Exception as error:
    ...
    def assign_ticket(self, ticket_key: str, assignee: str) -> dict:
        """
        "-1", the issue is assigned to the default assignee for the project.
        None, the issue is set to unassigned.
