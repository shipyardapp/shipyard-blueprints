...
from legacy_snowflake.cli import errors, utils
import warnings
import numpy as np
...
def convert_to_parquet(source_full_path, table_name, snowflake_datatypes):
    ...
    df.to_parquet(
        parquet_path, compression="gzip", write_index=False, write_metadata_file=False
    )
    return parquet_path
...
def upload_data(
    ...
    try:
        ...
        if "This session does not have a current schema." in str(pg_e):
            print(
                f"The schema provided either does not exist or your user does not have access to it. If no schema was provided, no default schema exists."
            )
            print(pg_e)
...
def main():
    ...
    try:
        if args.private_key_path != "":
            ...
            private_key = utils.decode_rsa(
                rsa_key=args.private_key_path, passphrase=args.private_key_passphrase
            )
        ...
        if "Incorrect username or password" in str(db_e):
            print(
                f"Invalid username or password. Please check for typos and try again."
            )
            print(db_e)
