...
class TableauClientConnectTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.client = TableauClient(
            self.username, self.password, self.server_url, self.site_id
        )
    ...
    def test_connect_with_invalid_username(self):
        assert (
            TableauClient(
                "invalid_username", self.password, self.server_url, self.site_id
            ).connect("access_token")
            == 1
        )
        assert (
            TableauClient("", self.password, self.server_url, self.site_id).connect(
                "access_token"
            )
            == 1
        )
    ...
    def test_connect_with_invalid_password(self):
        assert (
            TableauClient(
                self.username, "invalid_password", self.server_url, self.site_id
            ).connect("access_token")
            == 1
        )
        assert (
            TableauClient(self.username, "", self.server_url, self.site_id).connect(
                "access_token"
            )
            == 1
        )
    ...
    def test_connect_with_invalid_server_url(self):
        assert (
            TableauClient(
                self.username, self.password, "invalid_server_url", self.site_id
            ).connect("access_token")
            == 1
        )
        assert (
            TableauClient(self.username, self.password, "", self.site_id).connect(
                "access_token"
            )
            == 1
        )
