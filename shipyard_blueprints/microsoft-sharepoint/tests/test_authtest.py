...
def auth():
    return ["python3", "shipyard_microsoft_sharepoint/cli/authtest.py"]
...
def test_authtest_bad_client_id(auth, monkeypatch):
    monkeypatch.setenv("SHAREPOINT_CLIENT_ID", "bad_client_id")
    result = subprocess.run(auth, capture_output=True)
    assert result.returncode == 1


def test_authtest_bad_client_secret(auth, monkeypatch):
    monkeypatch.setenv("SHAREPOINT_CLIENT_SECRET", "bad_client_secret")
    result = subprocess.run(auth, capture_output=True)
    assert result.returncode == 1


def test_authtest_bad_tenant(auth, monkeypatch):
    monkeypatch.setenv("SHAREPOINT_TENANT", "bad_tenant")
    result = subprocess.run(auth, capture_output=True)
    assert result.returncode == 1
