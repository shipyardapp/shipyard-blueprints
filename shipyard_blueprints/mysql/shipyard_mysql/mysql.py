...
class MySqlClient(Database):
    ...
    def __init__(self, user: str, pwd: str, host: str, database: str,port:int = 3306, url_params=None) -> None:
        self.user = user
        self.pwd = pwd
        self.host = host
        self.port = port
        ...
        super().__init__(user = user, pwd = pwd, host = host, database = database, port = port, url_params = url_params)
