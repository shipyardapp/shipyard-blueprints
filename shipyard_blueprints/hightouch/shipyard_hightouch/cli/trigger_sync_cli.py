...
def generate_pickle_file(sync_run_id):
    ...
    base_folder_name = shipyard.logs.determine_base_artifact_folder("hightouch")
...
def get_args():
    ...
    parser.add_argument(
        "--full-resync", dest="full_resync", default=False, required=False
    )
...
def main():
    ...
    if args.wait_for_completion == "TRUE" and (0 < int(args.poke_interval) <= 60):
        ...
        hightouch.logger.error("Poke interval must be between 1 and 60 minutes")
