    ...
    def _determine_connection_status(self, response):
        ...
        if status_code == 401:
            if "Invalid token" in user_message:
                raise ExitCodeException(
                    f"The Service Token provided was invalid. Check to make sure there are no typos or preceding/trailing spaces. dbt API says: {user_message}",
...
if __name__ == "__main__":
    ...
    run_id = 266952652
    load_dotenv("../tests/.env.py")
    client = DbtClient(os.getenv("DBT_API_KEY"), os.getenv("DBT_ACCOUNT_ID"))
    # print(client.trigger_sync(os.getenv("DBT_JOB_ID")))
    # print(client.determine_sync_status(run_id))

    print(client.get_artifact_details(run_id))
    print(
        client.download_artifact(
            run_id, "compiled/jaffle_shop/models/customers.sql", os.getcwd()
        )
    )
