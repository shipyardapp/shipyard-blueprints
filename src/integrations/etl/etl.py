...
class EtlMeta(type):
    def __new__(cls, name, bases, body):
        ...
        if 'determine_sync_status' not in body:
            logging.error("verify_sync_status is a required method for this class")
            raise TypeError()
...
class Etl(metaclass = EtlMeta):
    ...
    def __init__(self, vendor:str, access_token:str) -> None:
        ...
        stream_format = logging.Formatter(format='%(asctime)s - %(levelname)s - %(message)s', datefmt='%d-%b-%y %H:%M:%S')
